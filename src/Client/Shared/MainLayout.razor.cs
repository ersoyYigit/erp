...
using ArdaManager.Client.Infrastructure.Settings;
...
namespace ArdaManager.Client.Shared
{
    public partial class MainLayout : IDisposable
    {
        ...
        private async Task LoadDataAsync()
        {
            ...
            if (user.Identity?.IsAuthenticated == true)
            {
                ...
                {
                    _snackBar.Add(localizer["Oturum süreniz dolduğu için çıkış yaptınız, lütfen yeniden giriş yapın."], Severity.Error);
        ...
        private bool _messageDrawerOpen = false;
        ...
        {
            _currentTheme = VappTheme.DefaultTheme;
            ...
            hubConnection.On<string, string, string>(ApplicationConstants.SignalR.ReceiveChatNotification, (message, receiverUserId, senderUserId) =>
            {
                if (CurrentUserId == receiverUserId)
                {
                    ...
                    _snackBar.Add(message, Severity.Info, config =>
                    {
                        ...
                        config.Action = localizer["GÖRÜNTÜLE?"];
                        config.ActionColor = Color.Primary;
                        config.Onclick = snackbar =>
                        {
                            //_navigationManager.NavigateTo($"chat/{senderUserId}");
                            _navigationManager.NavigateTo($"/catalog/templateworkorders/");
            ...
            hubConnection.On<string,string, string, string>(ApplicationConstants.SignalR.ReceiveDocStateNotification, (message,link, receiverUserId, senderUserId) =>
            {
                if (CurrentUserId == receiverUserId)
                {
                    _jsRuntime.InvokeAsync<string>("PlayAudio", "notification");
                    _snackBar.Add(message, Severity.Info, config =>
                    {
                        config.VisibleStateDuration = 18000;
                        config.HideTransitionDuration = 500;
                        config.ShowTransitionDuration = 500;
                        config.Action = localizer["Görüntüle?"];
                        config.ActionColor = Color.Primary;
            ...
            hubConnection.On(ApplicationConstants.SignalR.ReceiveRegenerateTokens, async () =>
            {
                try
                {
                    ...
                    {
                        _snackBar.Add(localizer["Oturum yenilendi."], Severity.Success);
                ...
                catch (Exception ex)
                {
                    ...
                    _snackBar.Add(localizer["Çıkış yaptınız."], Severity.Error);
                    await _authenticationManager.Logout();
                ...
                if (CurrentUserId != userId)
                {
                    ...
                    if (rolesResponse.Succeeded)
                    {
                        ...
                        if (role != null)
                        {
                            ...
                            {
                                _snackBar.Add(localizer["Yetkileriniz değiştiği için çıkış yaptınız, lütfen yeniden oturum açın."], Severity.Error);
        ...
        private void Logout()
        {
            ...
            {
                {nameof(Dialogs.Logout.ContentText), $"{localizer["Oturum Kapatılıyor"]}"},
            ...
            var options = new DialogOptions { CloseButton = true, MaxWidth = MaxWidth.Small, FullWidth = true };
             ...
             _dialogService.Show<Dialogs.Logout>(localizer["Oturumu Kapat"], parameters, options);
        }
        ...
        private void MessageDrawerToggle()
        {
            _messageDrawerOpen = !_messageDrawerOpen;
        }
        ...
        {
            if (firstRender)
                await _jsRuntime.InitializeMudBlazorExtensionsAsync();
