    ...
    internal class GetDashboardDataQueryHandler : IRequestHandler<GetDashboardDataQuery, Result<DashboardDataResponse>>
    {
        ...
        public async Task<Result<DashboardDataResponse>> Handle(GetDashboardDataQuery query, CancellationToken cancellationToken)
        {
            var response = new DashboardDataResponse
            {
                ...
                CompanyCount = await _unitOfWork.Repository<Company>().Entities.CountAsync(cancellationToken),
            ...
            double[] companiesFigure = new double[13];
            double[] patternsFigure = new double[13];
            ...
            for (int i = 1; i <= 12; i++)
            {
                ...
                companiesFigure[i - 1] = await _unitOfWork.Repository<Company>().Entities.Where(x => x.CreatedOn >= filterStartDate && x.CreatedOn <= filterEndDate).CountAsync(cancellationToken);
            ...
            response.DataEnterBarChart.Add(new ChartSeries { Name = _localizer["Companies"], Data = companiesFigure });
