...
namespace ArdaManager.Application.Features.Products.Queries.Export
{
        ...
        public async Task<Result<string>> Handle(ExportProductsQuery request, CancellationToken cancellationToken)
        {
            ...
            var mapper =
                new Dictionary<string, Func<Product, object>>
                {
                    ...
                    { _localizer["Type"], item => item.Type.ToDescriptionStringVapp() },
            ...
            var data = await _excelService.ExportAsync(products, mappers: mapper, sheetName: _localizer["Products"]);
