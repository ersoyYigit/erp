...
using ArdaManager.Infrastructure.Specifications;
...
namespace ArdaManager.Infrastructure.Services.Identity
{
    public class UserService : IUserService
    {
        private readonly UserManager<VappUser> _userManager;
        ...
        public UserService(
            UserManager<VappUser> userManager,
            IMapper mapper,
                ...
                if (result.Succeeded)
                {
                    ...
                    if (!request.AutoConfirmEmail)
                    {
                        ...
                        var mailRequest = new MailRequest
                        {
                            From = "admin@vapp.com",
                            To = user.Email,
                            Body = string.Format(_localizer["Lütfen <a href='{0}'>buraya tıklayarak</a> hesabınızı aktive edin."], verificationUri),
                            Subject = _localizer["Kullanıcı oluşturmayı onaylayın"]
                        };
                        BackgroundJob.Enqueue(() => _mailService.SendAsync(mailRequest));
                        return await Result<string>.SuccessAsync(user.Id, string.Format(_localizer["Kullanıcı {0} kaydedildi. Lütfen e-posta adresinizi doğrulayın!"], user.UserName));
                    }
                    return await Result<string>.SuccessAsync(user.Id, string.Format(_localizer["Kullanıcı {0} Kaydedildi."], user.Email));
                }
                else
                {
        ...
        private async Task<string> SendVerificationEmail(VappUser user, string origin)
        {
        ...
        public async Task<string> ExportToExcelAsync(string searchString = "")
        {
            ...
            var result = await _excelService.ExportAsync(users, sheetName: _localizer["Users"],
                mappers: new Dictionary<string, Func<VappUser, object>>
                {
        ...
        public async Task<IResult<List<UserResponse>>> GetUsersInRoleAsync(string roleName)
        {
