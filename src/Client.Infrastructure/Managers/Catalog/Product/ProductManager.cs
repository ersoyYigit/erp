...
namespace ArdaManager.Client.Infrastructure.Managers.Catalog.Product
{
    public class ProductManager : IProductManager
    {
        ...
        public async Task<IResult<List<ProductSearchResultDto>>> GetFilteredAsync(string searchTerm)
        {
            var response = await _httpClient.GetAsync(Routes.ProductsEndpoints.GetFiltered(searchTerm));
            return await response.ToResult<List<ProductSearchResultDto>>();
        }
    }
}